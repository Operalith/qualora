...
func ParseOpenAPISpec(raw string, sourceURL string, apiBaseURL string) (*parsedOpenAPISpec, error) {
	if len([]byte(raw)) > maxStoredSpecBytes {
		return nil, fmt.Errorf("OpenAPI document is too large for the v0.8 alpha import limit")
	}
	...
	if !strings.HasPrefix(version, "3.") {
		return nil, fmt.Errorf("only OpenAPI 3.x documents are supported in v0.8 alpha")
	}
...
func classifyOperation(pathName string, method string, pathItem map[string]any, operation map[string]any, rootSecurity any, hasRootSecurity bool) APIOperation {
	...
	if _, ok := openAPISafeMethods[method]; !ok {
		apiOperation.SkipReason = fmt.Sprintf("method %s is not read-only in v0.8", method)
		return apiOperation
	}
