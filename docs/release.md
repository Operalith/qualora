...
Qualora v0.16.0-alpha is the sixteenth public alpha release. It adds Interactive Safe Explorer while keeping local auth, browser/API smoke, credential profiles, authorization checks, application discovery, passive quality checks, guided onboarding, AI analysis, Safe QA Runs, and approved safe plan execution intact.
...
- The dashboard shows the `v0.16.0-alpha` badge, quick-start actions, status indicators, recent projects, and recent Safe QA runs.
...
- The reports landing page lists recent browser, API, discovery, Safe Explorer, quality, and Safe QA reports.
...
git status --short
git add .
git commit -m "feat: add interactive safe explorer for v0.16.0-alpha"
git tag -a v0.16.0-alpha -m "v0.16.0-alpha"
git push origin main
git push origin v0.16.0-alpha
```
...
Use [release-notes/v0.16.0-alpha.md](release-notes/v0.16.0-alpha.md) as the release body.
