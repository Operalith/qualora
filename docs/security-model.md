...
Qualora is security-adjacent automation. The v0.21.0-alpha safety model is intentionally conservative.
...
Application discovery in v0.21 is deterministic and safe by default. It is intended to build a lightweight application map, not to perform uncontrolled browser autonomy.
...
Interactive Safe Explorer in v0.21 is deterministic and safe by default. It is intended to demonstrate bounded, human-understandable page exploration, not autonomous browser control.
...
Quality checks in v0.21 are deterministic browser-worker observations. They are intended to surface obvious front-end quality issues, not to perform penetration testing, WCAG certification, Lighthouse audits, or exhaustive performance analysis.
...
The API worker and v0.21 control-plane API smoke executor validate `api_base_url`, `openapi_url`, imported OpenAPI URLs, OpenAPI server URLs, and every executed OpenAPI operation URL against the same host policy.
...
The policy engine must reject external navigation when same-origin mode is enabled, disallowed hosts, sensitive query strings, destructive or mutating labels/paths, unsupported actions, duplicate loop targets, unsafe form submission, payload/fuzzing/exploit language, and arbitrary selectors that are not recognized as safe supported actions.

## Safe QA Runs

Safe QA Runs in v0.21 orchestrate discovery, AI test planning, and safe test plan execution without changing the safety boundary.
