...
- `smoke.py`: end-to-end browser, OpenAPI import, safe API smoke, AI-analysis, AI test plan, and safe execution smoke test driver. It prints JSON and HTML report URLs, validates HTML report export, validates API result rows, validates skipped unsafe API operations, validates screenshot evidence download, verifies AI analysis with the fake provider, and executes a deterministic safe browser plan.
